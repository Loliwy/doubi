...
Download_mtproxy(){
	...
	git clone git://github.com/TelegramMessenger/MTProxy.git
...
Download_secret(){
	...
	wget --no-check-certificate -q "git://core.telegram.org/getProxySecret" -O "${mtproxy_secret}"
...
Download_multi(){
	...
	wget --no-check-certificate -q "git://core.telegram.org/getProxyConfig" -O "${mtproxy_multi}"
...
Service_mtproxy(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate "git://raw.githubusercontent.com/ToyoDAdoubi/doubi/master/service/mtproxy_centos" -O /etc/init.d/mtproxy; then
		...
		if ! wget --no-check-certificate "git://raw.githubusercontent.com/ToyoDAdoubi/doubi/master/service/mtproxy_debian" -O /etc/init.d/mtproxy; then
			echo -e "${Error} MTProxy服务 管理脚本下载失败 !"
			rm -rf "${file}"
			exit 1
		fi
